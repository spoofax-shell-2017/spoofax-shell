...
public class StyleFunction extends ContextualSpoofaxFunction<ParseResult, StyleResult> {
    ...
    @Override
    protected FailOrSuccessResult<StyleResult, IResult> applyThrowing(IContext context, ParseResult parseResult)
            throws Exception {
        ...
        final Iterable<IRegionCategory<IStrategoTerm>> categories =
                CategorizerValidator.validate(categorizer.categorize(lang, spoofaxParseUnit));

        Iterable<IRegionStyle<IStrategoTerm>> regions = stylerService.styleParsed(context.language(), categories);

        regions.forEach(region -> {
            ISourceLocation loc = tracer.location(region.fragment());
            ISourceRegion srcRegion = loc.region();

            FileObject fileObj = loc.resource();

//            try {
//
//                fileObj.getContent().getInputStream();
//
//                getInputStream().read(null, srcRegion., 0);
//            } catch (IOException e) {
...
//            }
        ...
        return FailOrSuccessResult.ofSpoofaxResult(resultFactory.createStyleResult(regions, spoofaxParseUnit));
